...
class Translator:
    ...
    TEMPLATES = {
        ...
        'fr': {
            ...
            'vigil_generic_full_name': 'la {name}',
            'ordinal_sunday_full_name': '{ordinal} Dimanche de l\'{season}',
            'ordinal_sunday_after_full_name': '{ordinal} Dimanche après l\'{event}',
            'last_sunday_full_name': 'Dernier Dimanche après l\'{event}',
            'feria_in_lent': '{weekday} de la {ordinal} semaine de Carême',
            'feria_after_ash_wednesday': '{weekday} après les Cendres',
            'feria_in_passion_week': '{weekday} de la semaine de la Passion',
            'class_feria': '{name} est une {type} de Classe {rank}.',
            'liturgical_color': 'La couleur liturgique est le {color}.',
            'outranking': '{feast} est remplacé par {outranking_feast}.',
            'outranking_this_year': (
                'Cette année {feast} est remplacé par {outranking_feast}.'),
            ...
            'lent_commemoration': (
                'Comme {feast} tombe pendant le Carême, il est ordinairement '
            ...
            'calendar_desc': (
                'Calendrier liturgique utilisant les rubriques de 1962.'),
        },
        'ja': {
    ...
    def _load_ordinals(self):
        if self.lang == 'ja':
            ...
            self.ordinals = {
                ...
                22: 'Vingt-deuxième', 23: 'Vingt-troisième',
                24: 'Vingt-quatrième', 25: 'Vingt-cinquième',
                26: 'Vingt-sixième', 27: 'Vingt-septième',
            }
        else:
            self.ordinals = {
    ...
    def _load_csv(self, resource_path, en_col, ja_col, encoding=None):
        if encoding is None:
            if 'ja' in resource_path:
                encoding = 'shift_jis'
            else:
                encoding = 'utf-8'
    ...
    def format_feast_full_name(self, name, rank):
        ...
        if self.lang == 'ja':
            ...
            # Skip if it already contains Dimanche or Fête
            if is_generic_sunday or is_already_feast:
                return translated_name
            ...
            if translated_name.upper().startswith(vowels):
                if rank == 4:
                    return f"la Commémoraison de l'{translated_name}"
                else:
                    return f"la Fête de l'{translated_name}"
            ...
            if article == "le ":
                if rank == 4:
                    return f"la Commémoraison du {translated_name}"
                else:
                    return f"la Fête du {translated_name}"

            if rank == 4:
                return self.templates['commemoration_full_name'].format(
                    name=article + translated_name)
            return self.templates['feast_full_name'].format(
                name=article + translated_name)
